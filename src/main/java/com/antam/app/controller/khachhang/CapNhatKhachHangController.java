...
public class CapNhatKhachHangController implements Initializable {
    ...
    private void openCapNhatKhachHangDialog(KhachHang khachHang) {
        try {
            javafx.fxml.FXMLLoader loader = new javafx.fxml.FXMLLoader(
                    getClass().getResource("/com/antam/app/views/khachhang/dialog/capNhatKhachHangForm.fxml")
            );
            javafx.scene.control.DialogPane dialogPane = loader.load();

            // Lấy controller từ FXML loader
            SuaKhachHangFormController controller = loader.getController();

            // Truyền dữ liệu khách hàng vào controller
            controller.setKhachHang(khachHang);

            // Thiết lập callback để reload bảng khi lưu thành công
            controller.setOnSaveListener(() -> {
                // Reload dữ liệu sau khi lưu thành công
                loadDataFromDB();
            });
